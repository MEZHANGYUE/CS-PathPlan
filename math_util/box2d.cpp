...
double PtSegDistance(
  double query_x, double query_y, double start_x, double start_y, double end_x, double end_y,
  double length)
{
...
void Box2d::InitCorners()
{
  ...
  for (size_t i = 0; i < 4; ++i) {
    max_x_ = std::fmax(corners_[i].x(), max_x_);
    min_x_ = std::fmin(corners_[i].x(), min_x_);
    max_y_ = std::fmax(corners_[i].y(), max_y_);
    min_y_ = std::fmin(corners_[i].y(), min_y_);
  }
  // gjk_data_ = GJKDistanceData(GetAllCorners());
}
...
LineSegment2d Box2d::axis() const
{
  const auto heading_vec = Vec2d::CreateUnitVec2d(heading_);
  return LineSegment2d(center_ - heading_vec * half_length_, center_ + heading_vec * half_length_);
}
...
void Box2d::GetAllCorners(std::vector<Vec2d> * const corners) const
{
  ...
  corners->assign(corners_, corners_ + 4);
}

std::vector<Vec2d> Box2d::GetAllCorners() const
{
  return std::vector<Vec2d>(corners_, corners_ + 4);
}
...
{
  const double nearest_distance = DistanceTo(point);
  return std::pow(nearest_distance, 2);
}

// bool Box2d::HasOverlap(const Polyline2d & polyline) const
// {
//   return std::any_of(
//     polyline.segments().begin(), polyline.segments().end(),
//     [this](const auto & line_segment) { return HasOverlap(line_segment); });
// }
...
bool Box2d::HasOverlap(const LineSegment2d & line_segment) const
{
  ...
  if (
    std::fmax(line_segment.start().x(), line_segment.end().x()) < min_x() ||
    ...
    switch (gx2 * 3 + gy2) {
      ...
      case 2:
        return (x1 > x2) ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2, line_segment.length())
                         : PtSegDistance(box_x, box_y, x1, y1, x2, y2, line_segment.length());
      case -1:
        return CrossProd<double>({x1, y1}, {x2, y2}, {box_x, -box_y}) >= 0.0
                 ? 0.0
                 : PtSegDistance(box_x, -box_y, x1, y1, x2, y2, line_segment.length());
      case -4:
        return CrossProd<double>({x1, y1}, {x2, y2}, {box_x, -box_y}) <= 0.0
                 ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2, line_segment.length())
                 : (CrossProd<double>({x1, y1}, {x2, y2}, {-box_x, box_y}) <= 0.0
                      ? 0.0
                      : PtSegDistance(-box_x, box_y, x1, y1, x2, y2, line_segment.length()));
    }
  } else {
...
// double Box2d::DistanceTo(const Box2d & box) const { return gjk(gjk_data_.bd, box.gjk_data().bd); }
...
// {
//   return util::StrCat(
//     "box2d ( center = ", center_.DebugString(), "  heading = ", heading_, "  length = ", length_,
//     "  width = ", width_, " )");
// }
